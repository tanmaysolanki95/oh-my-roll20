...
- **Token** — a circular canvas shape representing a character. Has HP, color, position, visibility, and optional size override.
...
| `src/components/map/TokenShape.tsx` | Single token shape with drag, HP, and +/- controls |
...
| `src/components/session/TokenPanel.tsx` | Sidebar token list: add, visibility toggle, delete, per-token size |
...
| `supabase/migrations/` | Incremental changes — apply in order (001 → 007) after schema.sql |

---

## RLS rules (as of migrations 001–006)

| Operation | Who |
|---|---|
| INSERT token | Session owner, OR any authenticated user where `owner_id = auth.uid()` |
| UPDATE token | Token owner OR session owner |
...
1. Add column to `sessions` in a new migration file (e.g. `008_my_setting.sql`)
