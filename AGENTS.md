...
**Drag ghost + boundary enforcement** — while `draggingTokenId` is set, a fixed-position ghost `<div>` rendered via `ReactDOM.createPortal` to `document.body` follows the cursor (tracked via `onPointerMove` on `SessionView`'s root div). The ghost shows the token's color circle + name. `MapCanvas` fires `onDropValidChange(boolean | null)` on `onPointerMove` (valid when cursor is inside `imageBounds`) and `onDropValidChange(null)` on `onPointerLeave`. `SessionView` owns `isDropValid: boolean | null` — `null` = neutral (white), `false` = invalid (red, 45% opacity). Drops outside `imageBounds` or when no map is loaded are rejected in `handleCanvasPointerUp` (returns early). `MapCanvas` sets `cursor: grabbing | no-drop` on its container div during drag.
...
- **`isDropValid` is `boolean | null`, not `boolean`** — `null` means "cursor not over canvas / validity unknown" and renders the ghost as neutral (white). `false` means explicitly invalid (red). Never use a plain boolean for this state — `null` and `false` have different meanings.
- **`onDropValidChange` accepts `boolean | null`** — `MapCanvas` passes `null` from `handleCanvasPointerLeave` (not `false`) so the ghost returns to neutral when the cursor leaves the canvas. Pass `null`, not `false`, for "unknown" validity.
- **Ghost must use `createPortal(..., document.body)`** — renders above all stacking contexts. `pointer-events: none` is required so the ghost div doesn't intercept pointer events and break drop detection.
